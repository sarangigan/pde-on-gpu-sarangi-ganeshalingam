...
@views function porous_convection_2D(; nx = 10, ny = 10, nt = 10, do_vis = false; do_check = false)
    # physics
    lx, ly     = 40.0, 20.0
    k_ηf       = 1.0
    αρgx, αρgy = 0.0, 1.0
    ...
    for it = 1:nt
        ...
        if do_vis
            ENV["GKSwstype"] = "nul"
            if (isdir("viz_out2d") == false) mkdir("viz_out2d") end
            loadpath = "viz_out2d/"
            ...
            if it % nvis == 0
                ...
                savefig(@sprintf("viz_out2d/%04d.png", iframe))
            end
        end
        iframe += 1
    end
