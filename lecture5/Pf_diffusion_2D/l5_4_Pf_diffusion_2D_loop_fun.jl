...
function Pf_diffusion_2D(;do_check = true, bench = :btool)
    # physics
    lx, ly = 20.0, 20.0
    k_ηf   = 1.0
    # numerics
    ...
    if bench == :loop
        ...
        t_toc_avg = mean(t_toc_arr)
        A_eff = 10 * (nx * ny) * sizeof(eltype(Float64)) / 1e9        # Effective main memory access per iteration [GB]
        T_eff = A_eff/t_toc_avg   # Effective memory throughput [GB/s]
            @printf("Time = %1.2e sec (@ %1.3e GB/s) \n", t_toc_avg, T_eff)
        end
        return
    end


Pf_diffusion_2D(do_check = false, bench=:btool)