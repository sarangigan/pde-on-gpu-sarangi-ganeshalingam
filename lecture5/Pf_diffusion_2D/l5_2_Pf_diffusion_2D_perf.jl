...
function Pf_diffusion_2D(do_check = false, nx=512, ny=512)
...
for n_iter = 1:8
    ...
    nx_plot = n[n_iter]
    ny_plot = n[n_iter]
    T_eff = @belapsed Pf_diffusion_2D($do_check,$nx_plot,$ny_plot) 
    push!(T_eff_perf, T_eff)
end

plot(n, T_eff_perf, label="Kernel Based Approach", marker=:circle)
