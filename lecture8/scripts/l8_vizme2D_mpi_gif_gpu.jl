...
@views function vizme2D_mpi(nprocs)
    ...
    anim = @animate for it = 1:nt
        ...
        for ipx = 1:nprocs[1]
            for ipy = 1:nprocs[2]
                println(@sprintf("docs/mpi2D_out_C_%d_%04d.mat", (ip-1), it))
                file = matopen(@sprintf("docs/mpi2D_out_C_%d_%04d.mat", (ip-1), it))
                # file = matopen("scripts/mpi2D_out_C_$(ip-1)_it.mat")
        ...
        display(heatmap(C'; c=:turbo, title="diffusion 2D MPI with GPU, it = $it", opts...))
    end
    gif(anim,"docs/diffusion2d_mpi_gpu.gif";fps=30) 
    return
end
